 ...
 *
 *  Modify settings.csv and execute settings.py to generate
 *
 ***********************************************/
...
""".format(cname = row[colname].capitalize(), name = row[colname], dtype = typeToC(row[coltype]), deflt = row[coldefault], minv = row[colmin], maxv = row[colmax], desc = row[coldesc] )
  f.write(txt)

# Write all JSON Settings
f.write("  void setJSONSettings(DynamicJsonDocument &json) {\n")
for row in settings:
  f.write("    json[\"" + row[colname] + "\"] = " + row[colname] + ";\n");
f.write("  }\n")

# Read JSON Settings
f.write("\n  void loadJSONSettings(DynamicJsonDocument &json) {\n    JsonVariant v;\n")
for row in settings:
  f.write("    v = json[\"" + row[colname] + "\"]; if(!v.isNull()) set" + row[colname].capitalize() + "(v);\n");
f.write("  }\n")
...
f.write("\n  //Settings\n")
for row in settings:
  f.write("  " + typeToC(row[coltype]) + " " + row[colname] + " = " + row[coldefault] + "; //" + row[coldesc] + "\n")

f.write("\n  //Setting Arrays\n")
for row in settingsarrays:
  f.write("  " + typeToC(row[coltype]) + " " + row[colname] + "; //" + row[coldesc] + "\n")
