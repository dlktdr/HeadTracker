...
//*************************************************
// Added from other files + Needed to make compile
...
// Modified for FIFO lib available
FIFO<uint8_t> btTxFifo(BT_TX_FIFO_SIZE);
FIFO<uint8_t> btRxFifo(BT_RX_FIFO_SIZE);
#define hasSpace(x) available() >= x
#define push(x) put(x)

extern FIFO<uint8_t> btTxFifo;
extern FIFO<uint8_t> btRxFifo;
...
#define RTOS_WAIT_MS(x) ThisThread::sleep_for(std::chrono::milliseconds( x ));
