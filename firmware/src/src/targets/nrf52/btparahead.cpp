...
static ssize_t write_json(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf,
                          uint16_t len, uint16_t offset, uint8_t flags);
static ssize_t read_json(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf,
                         uint16_t len, uint16_t offset);
...
BT_GATT_SERVICE_DEFINE(
    ...
    BT_GATT_CHARACTERISTIC(&jsonuuid.uuid, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
