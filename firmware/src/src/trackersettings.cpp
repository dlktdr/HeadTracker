...
int TrackerSettings::Rll_min() const { return rll_min; }
...
int TrackerSettings::Rll_max() const { return rll_max; }
...
float TrackerSettings::Rll_gain() const { return rll_gain; }
...
int TrackerSettings::Rll_cnt() const { return rll_cnt; }

void TrackerSettings::setRll_cnt(int value)
{
  if (value < MIN_CNT) value = MIN_CNT;
  if (value > MAX_CNT) value = MAX_CNT;
  rll_cnt = value;
}

int TrackerSettings::Pan_min() const { return pan_min; }
...
int TrackerSettings::Pan_max() const { return pan_max; }
...
float TrackerSettings::Pan_gain() const { return pan_gain; }
...
int TrackerSettings::Pan_cnt() const { return pan_cnt; }
...
int TrackerSettings::Tlt_min() const { return tlt_min; }
...
int TrackerSettings::Tlt_max() const { return tlt_max; }
...
float TrackerSettings::Tlt_gain() const { return tlt_gain; }
...
int TrackerSettings::Tlt_cnt() const { return tlt_cnt; }
...
void TrackerSettings::setServoreverse(char value) { servoreverse = (int)value; }
...
bool TrackerSettings::isRollReversed() { return (servoreverse & HT_ROLL_REVERSE_BIT); }

bool TrackerSettings::isTiltReversed() { return (servoreverse & HT_TILT_REVERSE_BIT); }

bool TrackerSettings::isPanReversed() { return (servoreverse & HT_PAN_REVERSE_BIT); }

int TrackerSettings::panCh() const { return panch; }

void TrackerSettings::setPanCh(int value)
{
  if ((value > 0 && value < 17) || value == -1) panch = (int)value;
}

int TrackerSettings::tiltCh() const { return tltch; }

void TrackerSettings::setTiltCh(int value)
{
  if ((value > 0 && value < 17) || value == -1) tltch = (int)value;
}

int TrackerSettings::rollCh() const { return rllch; }

void TrackerSettings::setRollCh(int value)
{
  if ((value > 0 && value < 17) || value == -1) rllch = (int)value;
}

int TrackerSettings::alertCh() const { return alertch; }

void TrackerSettings::setAlertCh(int value)
{
  if ((value > 0 && value < 17) || value == -1) alertch = (int)value;
}

//----------------------------------------------------------------------------------------
// Remappable Buttons + PPM Output Pin + Bluetooth

int TrackerSettings::buttonPin() const { return buttonpin; }
...
int TrackerSettings::ppmOutPin() const { return ppmoutpin; }
...
int TrackerSettings::resetCntPPM() const { return rstppm; }

void TrackerSettings::setResetCntPPM(int value)
{
  if ((value >= 1 && value <= 16) || value == -1) rstppm = value;
}

bool TrackerSettings::resetOnWave() const { return rstonwave; }

void TrackerSettings::setResetOnWave(bool value) { rstonwave = value; }
...
{
  if ((funct >= AUX_GYRX && funct <= BT_RSSI) || funct == -1) aux0func = funct;
}

void TrackerSettings::setAuxFunc1(int funct)
{
  if ((funct >= AUX_GYRX && funct <= BT_RSSI) || funct == -1) aux1func = funct;
}

void TrackerSettings::setAuxFunc2(int funct)
{
  if ((funct >= AUX_GYRX && funct <= BT_RSSI) || funct == -1) aux2func = funct;
}

void TrackerSettings::setAnalog4Ch(int channel)
{
  if ((channel > 0 && channel <= 16) || channel == -1) an4ch = channel;
}

void TrackerSettings::setAnalog5Ch(int channel)
{
  if ((channel > 0 && channel <= 16) || channel == -1) an5ch = channel;
}

void TrackerSettings::setAnalog6Ch(int channel)
{
  if ((channel > 0 && channel <= 16) || channel == -1) an6ch = channel;
}

void TrackerSettings::setAnalog7Ch(int channel)
{
  if ((channel > 0 && channel <= 16) || channel == -1) an7ch = channel;
}
...
void TrackerSettings::loadJSONSettings(DynamicJsonDocument &json)
{
  ...
  v = json["butlngps"];
  if (!v.isNull()) butlngps = v;
  v = json["rstontlt"];
  if (!v.isNull()) rstontlt = v;

  v = json["ppmininvert"];
  if (!v.isNull()) setInvertedPpmIn(v);
  v = json["ppmoutinvert"];
  if (!v.isNull()) setInvertedPpmOut(v);

  // Reset center on PPM Ch > 1800us
  v = json["rstppm"];
  if (!v.isNull()) setResetCntPPM(v);

  // PPM Data
  v = json["ppmfrm"];
  if (!v.isNull()) setPPMFrame(v);
  v = json["ppmchcnt"];
  if (!v.isNull()) setPpmChCount(v);
  v = json["ppmsync"];
  if (!v.isNull()) setPPMSync(v);

  // PWM Settings
  v = json["pwm0"];
  if (!v.isNull()) setPWMCh(0, v);
  v = json["pwm1"];
  if (!v.isNull()) setPWMCh(1, v);
  v = json["pwm2"];
  if (!v.isNull()) setPWMCh(2, v);
  v = json["pwm3"];
  if (!v.isNull()) setPWMCh(3, v);

  // SBUS Settings
  v = json["sbininv"];
  if (!v.isNull()) setInvertedSBUSIn(v);
  v = json["sboutinv"];
  if (!v.isNull()) setInvertedSBUSOut(v);
  v = json["sbrate"];
  if (!v.isNull()) setSBUSRate(v);

  // Analog Settings
  v = json["an4ch"];
  if (!v.isNull()) setAnalog4Ch(v);
  v = json["an4off"];
  if (!v.isNull()) setAnalog4Offset(v);
  v = json["an4gain"];
  if (!v.isNull()) setAnalog4Gain(v);
  v = json["an5ch"];
  if (!v.isNull()) setAnalog5Ch(v);
  v = json["an5off"];
  if (!v.isNull()) setAnalog5Offset(v);
  v = json["an5gain"];
  if (!v.isNull()) setAnalog5Gain(v);
  v = json["an6ch"];
  if (!v.isNull()) setAnalog6Ch(v);
  v = json["an6off"];
  if (!v.isNull()) setAnalog6Offset(v);
  v = json["an6gain"];
  if (!v.isNull()) setAnalog6Gain(v);
  v = json["an7ch"];
  if (!v.isNull()) setAnalog7Ch(v);
  v = json["an7off"];
  if (!v.isNull()) setAnalog7Offset(v);
  v = json["an7gain"];
  if (!v.isNull()) setAnalog7Gain(v);

  // AUX Settings
  v = json["aux0func"];
  if (!v.isNull()) setAuxFunc0(v);
  v = json["aux0ch"];
  if (!v.isNull()) setAuxFunc0Ch(v);
  v = json["aux1func"];
  if (!v.isNull()) setAuxFunc1(v);
  v = json["aux1ch"];
  if (!v.isNull()) setAuxFunc1Ch(v);
  v = json["aux2func"];
  if (!v.isNull()) setAuxFunc2(v);
  v = json["aux2ch"];
  ...
  v = json["so01"];
  if (!v.isNull()) magsioff[1] = v;
  v = json["so02"];
  if (!v.isNull()) magsioff[2] = v;
  v = json["so10"];
  if (!v.isNull()) magsioff[3] = v;
  v = json["so11"];
  if (!v.isNull()) magsioff[4] = v;
  v = json["so12"];
  if (!v.isNull()) magsioff[5] = v;
  v = json["so20"];
  if (!v.isNull()) magsioff[6] = v;
  v = json["so21"];
  if (!v.isNull()) magsioff[7] = v;
  v = json["so22"];
  if (!v.isNull()) magsioff[8] = v;
...
// Macro Expansion for Data Variables + Arrays
#define DV(DT, NAME, DIV, ROUND) array.add(#NAME);
  DATA_VARS
#undef DV

#define DA(DT, NAME, SIZE, DIV) array.add(#NAME);
  DATA_ARRAYS
#undef DA
}
