...
class TrackerSettings
{
 public:
  ...
  static constexpr float RECENTER_PULSE_DURATION =
      0.5;                                // (sec) pulse width of recenter signal to tx
  ...
  void setPPMFrame(uint16_t v)
  {
    if (v >= PPM_MIN_FRAME && v <= PPM_MAX_FRAME) ppmfrm = v;
  }

  uint16_t ppmSync() { return ppmsync; }
  void setPPMSync(uint16_t v)
  {
    if (v >= PPM_MIN_SYNC && v <= PPM_MAX_SYNC) ppmsync = v;
  }

  int ppmChCount() { return ppmchcnt; }
  void setPpmChCount(int v)
  {
    if (v > 0 && v < 17) ppmchcnt = v;
  }
  ...
  void setButtonPressMode(bool lngpresmd)
  {
    butlngps = lngpresmd;
  }  // True = Enable/Disable output on long press
  ...
  void setSBUSRate(uint8_t rate)
  {
    if (rate >= 30 && rate <= 150) sbrate = rate;
  }
