...
class MPU6886 {
   ...
   public:
    MPU6886() {}
    int Init(void);
    int getAccelAdc(int16_t* ax, int16_t* ay, int16_t* az);
    int getGyroAdc(int16_t* gx, int16_t* gy, int16_t* gz);
    void getTempAdc(int16_t* t);

    int getAccelData(float* ax, float* ay, float* az);
    int getGyroData(float* gx, float* gy, float* gz);
   ...
   private:
   private:
    int I2C_Read_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
                         uint8_t number_Bytes, uint8_t* read_Buffer);
    int I2C_Write_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
