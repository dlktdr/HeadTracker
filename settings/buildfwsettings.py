...
for row in settingsarrays:
  ...
  if "char" in row[coltype]:
    f.write("    strcpy(" + name + ",\"" + row[coldefault] + "\");\n")
  else:
...
""".format(cname = row[colname], name = row[colname].lower(), dtype = typeToC(row[coltype]), deflt = row[coldefault].lower(), minv = row[colmin], maxv = row[colmax], desc = row[coldesc])
  else:
    txt = """\
  // {desc}
  inline const {dtype}& get{cname}() {{return {name};}}
...
for row in settings:
  if "bool" in row[coltype]:
    dflt = row[coldefault].lower().strip()
  else:
    dflt = row[coldefault].strip()
  f.write("  " + typeToC(row[coltype]) + " " + row[colname].lower().strip())
  if row[coldefault] == "":
    f.write (";")
  else:
    f.write (" = " + dflt + ";")
